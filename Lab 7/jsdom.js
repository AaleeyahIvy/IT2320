...
//All event listeners go inside window.onload
    ...
    var btnsOrig =  document.getElementsByClassName("btnOrig");
    for( var i=0; i < btnsOrig.length; i++){
        btnsOrig[i].addEventListener("click", function(){
            changeButtonText(this);
        } )
    }
    var squares3 =  document.getElementsByClassName("square3");
    for( var i=0; i < squares3.length; i++){
        squares3[i].addEventListener("click", function(){
            changeSquare(this);
        } )
    }
}
